...
export default function AchievementModal({ cards }: { cards: AchievementCard[] }) {
  ...
  return (
    ...
    >
      <div
        className={`relative max-h-[85vh] w-full max-w-[44rem] overflow-y-auto border border-stone-200 bg-[#f6f3ec] shadow-[0_18px_48px_rgba(35,31,24,0.14)] transition-all duration-300 sm:p-0 ${
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-stone-200 bg-[#f6f3ec]/95 px-4 py-3 backdrop-blur-sm sm:px-6">
          ...
          <button
            ...
            onClick={closeModal}
            aria-label="關閉案例視窗"
            className="inline-flex items-center gap-2 border border-transparent px-2 py-1 font-sans text-[11px] font-bold uppercase tracking-[0.2em] text-gray-500 transition hover:text-ink"
          >
            <svg
